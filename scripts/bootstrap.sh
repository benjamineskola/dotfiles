...
test -x "$(command -v brew)" && brew bundle --global install

test -x "$(command -v bat)" && bat cache --build
test -x "$(command -v pip3)" && pip3 install -r requirements.txt
test -x "$(command -v yarn)" && xargs yarn global add <npm-globals.txt
